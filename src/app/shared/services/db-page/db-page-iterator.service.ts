...
export class DbPageIterator<T> {
    ...
    init(url: string, callback?: Function): this {
        this.addSyncTask(async () => {
            await this.download(url, callback);
        });
        return this;
    }

    //load all pages on source url
    loadAll(url: string, callback?: Function): this {
        this.addSyncTask(async () => {
            const it = new DbPageIterator<T>(this.retrieveData);

            it.init(url, callback);
            this.page = {
                'count': it.page.count,
                'next': it.page.next,
            ...
            while (it.hasNext()) {
                it.loadNext();
                next.push(...it.results);
            }

            while (it.hasPrevious()) {
                it.loadPrevious(() => it.results);
                prev.push(...it.results);
            }
    ...
    loadNext(callback?: Function, offset?: number): this {
        this.addSyncTask(async () => {
            ...
            for (let i = 0; i < offsetArg; i++) {
                if (this.hasNext()) {
                    if (i + 1 === offset) {
                        await this.download(this.page.next, callback);
                    } else {
                        await this.download(this.page.next, undefined);
                    }
                } else {
    ...
    loadPrevious(callback?: Function, offset?: number): this {
        this.addSyncTask(async () => {
            let offsetArg: number = offset === undefined ? 1 : offset;
            ...
            for (let i = 0; i < offsetArg; i++) {
                if (this.hasNext()) {
                    if (i + 1 === offset) {
                        await this.download(this.page.previous, callback);
                    } else {
                        await this.download(this.page.previous, null);
                    }
                } else {
    ...
    private async download(url: string, callback: Function): Promise<void> {
        return this.retrieveData.fetch(url).then(data => {
            this.page = data;
            callback();
        });
    }
